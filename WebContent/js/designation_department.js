...
function getOrgbyTypeAtLevel(id, level, stateCode) {
	 ...
	 * 
	 * if (hdnstateCode > 0) { stateCode = hdnstateCode; } else { stateCode = 0; }
	 */
/*displayLoadingImage();*/
...
function handleOrgTypeLevelSuccess(data) {
	...
	dwr.util.removeAllOptions('orgLocatedLevelCode');
	var dataq = [ {
		select : "Select"
	} ];
...
function handleOrgTypeLevelError() {
	dwr.util.removeAllOptions('olc');
	dwr.util.removeAllOptions('orgLocatedLevelCode');
	var dataq = [ {
		select : "Select"
	} ];
	dwr.util.addOptions('olc', dataq, "select");
	hideLoadingImage();
}
...
function handleOrgAtLevelSuccess(data) {
	var fieldId = 'orgLocatedLevelCode';
...
function getDesignationDetails(olc, locatedLevel) {
	clearAll();
	if (locatedLevel == "-1") {
		document.getElementById('errorLocatedLevel').innerHTML = "Please Select a Level.";
		return false;
	}
	displayLoadingImage();
	lgdDesignationDwr.getExistingDesignationsForDepartment(parseInt(olc), parseInt(locatedLevel), {
		async : true,
		callback : function(data) {
...
function clearAll() {
	document.getElementById('errorOrgType').innerHTML = "";
	document.getElementById('errorOlc').innerHTML = "";
	document.getElementById('errorLocatedLevel').innerHTML = "";
...
function getOtherDesignations() {
	...
	if (orgType == "0") {
		document.getElementById('errorOrgType').innerHTML = "Please select a Level.";
		return false;
	}

	var olc = document.getElementById('olc').value;
	if (olc == "0") {
		document.getElementById('errorOlc').innerHTML = "Please select a Organization.";
		return false;
	}

	var orgLocatedLevelCode = document.getElementById('orgLocatedLevelCode').value;
	if (orgLocatedLevelCode == "0") {
		document.getElementById('errorLocatedLevel').innerHTML = "Please select a Level.";
		return false;
	}
