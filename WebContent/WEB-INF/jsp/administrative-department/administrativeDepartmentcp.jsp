...
<script type="text/javascript">
	...
	$( document ).ready(function() {
		...
		$('#adminOrgDeptForm input[type=text]').attr("autocomplete",'off');
		 $("#alertboxbtnYES").click(function() {
			 saveData(_hirarchay_url);
		  });
		 
		 $("#alertboxbtnNO").click(function() {
			 saveData(_save_url);
		  });
	});
				   ...
				   <form:form id="adminOrgDeptForm" commandName="adminOrgDeptForm" class="form-horizontal">
							...
							<c:if test="${!isOrganizationFlow}">
								<div class="form-group">
								  ...
								  <div class="col-sm-6">
									...
									<label id="err_rMinistry" class="mandatory"></label>
								  </div>
							   </div>
							</c:if>
							<div class="form-group" id="divMinistry" style="display: none;">
							  ...
							  <div class="col-sm-6">
							  		<form:select path="ministryId" id="ministryId" class="form-control"  onchange="getDepartmentList1(this.value)" htmlEscape="true">
	                			    ...
	                			    <label id="err_ministry" class="mandatory"></label>
									<form:errors  path="ministryId" htmlEscape="true"  cssClass="mandatory"/>
						...
						<c:if test="${isOrganizationFlow}">
							<div class="form-group">
								  ...
								  <div class="col-sm-6">
								  		<form:select path="deptOrgCode"  id="deptOrgCode" class="form-control" htmlEscape="true">
		                			    ...
		                			    <label id="err_dept" class="mandatory"/>
										<form:errors htmlEscape="true" path="deptOrgCode" cssClass="mandatory"></form:errors>
								  </div>
							</div>
							<div class="form-group">
								  ...
								  <div class="col-sm-6">
								  		<form:select path="orgType" id="orgType" class="form-control" htmlEscape="true">
		                			    ...
		                			    <label id="err_org" class="mandatory"></label>
										<form:errors htmlEscape="true" path="orgType" cssClass="mandatory"></form:errors>
								  </div>
							</div>
						</c:if>
						...
						<c:if test="${not empty pageAccessLevel}">
							...
							<c:when test="${fn:contains(pageAccessLevel, 'X')}">
								...
								<c:when test="${fn:contains(pageAccessLevel, 'A')}">
									...
									<c:set var="titleLavelAdminNameArr" value="${fn:split(adminOrgDeptForm.parentHierarchy, '#')}" />
								...
								<c:when test="${fn:contains(pageAccessLevel, 'C')}">
									<c:set var="lblChooseLevelAll" value="Label.ALLCOUNTRY"></c:set>
									<c:set var="lblChooseLevelSpecific" value="Label.SPECCOUNTRY"></c:set>
									<c:set var="titleLavel" value="Label.COUNTRYLEVEL"></c:set>
									<c:set var="titleLavelAdminNameArr" value="${fn:split(adminOrgDeptForm.parentHierarchy, '#')}" />
									<c:set var="titleGeneral" value="Label.COUNTRYDEPT"></c:set>
						...
						<c:choose>
							...
							<c:otherwise>
								<c:if test="${!empty pageAccessLevel }">
									...
									<c:out value="(${titleLavelAdminNameArr[0]})"/>
									</c:if>
								</c:if>
							</c:otherwise>
						</c:choose>
					...
					<div class="form-group">
							  ...
							  <div class="col-sm-6">
							  	<form:select path="parentDepartmentName" id="parent" class="form-control"  style="width: 200px" htmlEscape="true">
									<form:option value="${parent}"><c:out value="${parent}" escapeXml="true"></c:out></form:option>
								</form:select>
							  		
									<form:errors  path="" htmlEscape="true"  cssClass="mandatory"/>
							  </div>
						</div>
					</c:if>
					...
					<div class="form-group">
						  ...
						  <div class="col-sm-6">
						  	<form:input path="departmentNameEnglish" id="deptNameEnglish"  class="form-control" htmlEscape="true" maxlength="200"/>
						  	<label id="err_deptName" class="mandatory"></label>
							<form:errors htmlEscape="true" path="departmentNameEnglish" class="mandatory"/>
						  </div>
					</div>
					<div class="form-group">
						  <label  class="col-sm-3 control-label" for="sel1"><spring:message code='${deptOrgNameLocal}' htmlEscape="true"/> </label>
						  <div class="col-sm-6">
						  <form:input path="departmentNameLocal" id="deptNameLocal"  class="form-control" htmlEscape="true" maxlength="80"/>
						  <form:errors htmlEscape="true" path="" class="mandatory"/>
						  </div>
					</div>
					<div class="form-group">
						  <label  class="col-sm-3 control-label" for="sel1"><spring:message code='${deptOrgNameShort}' htmlEscape="true"/> </label>
						  <div class="col-sm-6">
						  <form:input path="departmentShortName" id="sortNameDept"  class="form-control" htmlEscape="true" maxlength="10"/>
						  </div>
					</div>
                  	...
                  	<div class="box-header subheading">
                 		<h4 class="box-title"><spring:message htmlEscape="true"  code='${deptOrgOperationalAt}'/></h4>
              	    </div><!-- /.box-header -->
												...
												<c:if test="${ fn:contains(pageAccessLevel, 'S')
												    ...
												    <c:if test="${fn:contains(pageAccessLevel, 'D')																	
												  		...
												  		<form:checkbox  name="checkbox" id="ChkFullCoverage" value="Full" path="ChkFullCoveragestate" htmlEscape="true" onclick="StateFullPart();" />
																	...
																	<label id="err_statechk" class="mandatory"></label>
												  		</div>
													...
													<div class="ms_container row" >
														...
														<div class="ms_selection col-sm-5">
															<div class="form-group">
																...
																<label id="err_state" class="mandatory"></label>
															</div>
...
<!--  --------------------------------------------------------------------------------------- Urban Block start here      ---------------------------------------------------------------------------------  -->
		  <c:if test="${ fn:contains(pageAccessLevel, 'C')}">
		  <div class="box-body dept_list_button" id="tblStateSpecific" >
			<div class="ms_container row"  >
					<div class="ms_selectable col-sm-5 form-group">
						<label for="ddSourceVillageRLBS"><spring:message code="label.avilable.country.list" ></spring:message></label>
						<form:select path=""  id="sourceCountryList" multiple="true"  class="form-control" htmlEscape="true">
							<form:options items="${countryList}" itemValue="countryCode" itemLabel="countryName" htmlEscape="true"/>
						</form:select>
					</div>
					<div class="ms_buttons col-sm-2"><br>
						<button type="button" class="btn btn-primary btn-xs btn-block" onclick="moveElement('sourceCountryList','targetCountryList',null)"> &gt;&gt;</button>
						<button type="button" class="btn btn-primary btn-xs btn-block" onclick="moveElement('targetCountryList','sourceCountryList',null)"> &lt;&lt;</button>
					</div>
					<div class="ms_selection col-sm-5">
						<div class="form-group">
							<label for="ddDestVillageRLBs"><spring:message code="label.contribute.country.list" ></spring:message></label> 
								<form:select path="countryIds" id="targetCountryList" multiple="true"  class="form-control" htmlEscape="true"/>
							<label id="err_ulevel" class="mandatory"></label>	
						</div>
					</div>
				</div>	
			</div>
	...
	<div class="ms_container row" >
		...
		<div class="ms_selection col-sm-5">
			<div class="form-group">
				...
				<label id="err_dplevel" class="mandatory"></label>	
			</div>
		</div>
	</div>	
</div>
...
<c:if test="${ fn:contains(pageAccessLevel, 'Y') or fn:contains(pageAccessLevel, 'Z')}">
	<div class="col-sm-12">
		<form:checkbox path="DPChkfull" id="DPChkFull"   htmlEscape="true" onclick="showHideDiv('DPCHK');" value="DPFULL" />
		 ...
		 <label id="err_DPchk" class="mandatory"></label>
	</div>
	...
	<div class="ms_container row" >
		...
		<div class="ms_selection col-sm-5">
			<div class="form-group">
				...
				<label id="err_iplevel" class="mandatory"></label>	
			</div>
		</div>
	</div>	
...
<div class="ms_container row"  >
	...
	<div class="ms_selection col-sm-5">
		<div class="form-group">
			...
			<form:select path="vpIds" id="targetVpList" multiple="true"  class="form-control" htmlEscape="true"/></br>
			<label id="err_vplevel" class="mandatory"></label>	
		</div>
	</div>
</div>
</div>	
	...
	<c:if test="${isCenterUser == true}">
		<c:if test="${ fn:contains(pageAccessLevel, 'D')}">
			<div class="form-group"  id="tblStateList">
			  ...
			  <div class="col-sm-6">
				<form:select path="" htmlEscape="true" class="combofield" id="stateName" onchange="getDistrictsforVillageS();"></form:select>
		      </div>
		   </div>			
		</c:if>
	</c:if>
	 ...
	 <!-- Drop-down District with full Coverage used in Sub-district-->
			<c:if test="${ fn:contains(pageAccessLevel, 'T') or fn:contains(pageAccessLevel, 'B') or fn:contains(pageAccessLevel, 'V')}">
				<div class="col-sm-12" id="tblCheckCoverageDistrictFull">
						...
						<label id="err_districtchk" class="mandatory"></label>
				</div>						
			</c:if>
			...
			<c:if test="${isCenterUser == true}">
				<c:if test="${ fn:contains(pageAccessLevel, 'T')  or fn:contains(pageAccessLevel,'V')}">
					<div class="form-group" id="tblStateList1">
					  <label  class="col-sm-3 control-label" for="sel1"><spring:message htmlEscape="true"  code="Label.SELECTSTATE"/></label>
					  <div class="col-sm-6">
						<form:select path="" htmlEscape="true" class="combofield" id="stateName1" onchange="getDistrictsforVillageS();"></form:select>
				      </div>
				   </div>				              
				</c:if>
			</c:if>
			...
			<div class="ms_container row">
				...
				<div class="ms_selection col-sm-5">
					<div class="form-group">
						...
						<form:select path="districtIds" id="ddTargetDistrict" multiple="true"  class="form-control" htmlEscape="true"/></br>
						<label id="err_district" class="mandatory"></label>
					</div>
				</div>
			</div>
			</div>	
...
<c:if test="${isCenterUser == true}">
	<c:if test="${ fn:contains(pageAccessLevel, 'T') or fn:contains(pageAccessLevel, 'B') or fn:contains(pageAccessLevel, 'V')}">
		<div class="form-group" id="tblDistrictSelectBox1">
			  <label  class="col-sm-3 control-label" for="sel1"><spring:message htmlEscape="true"  code="Label.SELECTSTATE"/></label>
			  <div class="col-sm-6">
				<form:select path="" class="combofield" id="stateNameDistforSD" style="width:200px" onchange="getNotInDistrictList(this.value);" htmlEscape="true">
	...
	<div class="ms_container row"  >
		...
		<div class="ms_selection col-sm-5">
			<div class="form-group">
				...
				<form:select path="subdistrictIds" id="ddTargetDistrictSDLvl" multiple="true"  class="form-control" htmlEscape="true"/>
				<label id="err_subDistrict" class="mandatory"></label>
			</div>
		</div>
	</div>
</div>			
	...
	<div class="ms_container row"  >
			...
			<div class="ms_selection col-sm-5">
				<div class="form-group">
					...
					<form:select path="blockIds" id="ddTargetBlockSDLvl" multiple="true"   class="form-control" htmlEscape="true"/>
					<label id="err_block" class="mandatory"></label>
				</div>
			</div>
	</div>	
</div>
...
<c:if test="${ fn:contains(pageAccessLevel, 'V')}">
	...
	<div class="form-group" id="tblSubDistrictSelectState" >
	  <label  class="col-sm-3 control-label" for="sel1"><spring:message htmlEscape="true"  code="Label.SELECTSTATE"/></label>
	  <div class="col-sm-6">
		<form:select path="" style="width:200px" class="combofield" id="stateNameSubDistForVillage" onchange="getNotInDistrictListVillLvl(this.value);" htmlEscape="true">
			<form:option value="" htmlEscape="true"><spring:message htmlEscape="true"  code="Label.SELECT"/></form:option> 
		</form:select>
	   </div>
	</div>
	...
	<div id="tblDisttSubDisttVillLvlSelectBox">
		<div class="form-group" >
		  <label  class="col-sm-3 control-label" for="sel1"><spring:message htmlEscape="true" code="Label.SELECTDISTRICT"/></label>
		  <div class="col-sm-6">
			<form:select path="" id="dddistrictAtVillLvl" class="combofield"  style="width:200px" onchange="getSubdistrictAtVilllvl(this.value)" htmlEscape="true"></form:select>
		  </div>
		</div>
		<div class="form-group" >
		  <label  class="col-sm-3 control-label" for="sel1"><spring:message htmlEscape="true" code="Label.SELECTSUBDISTRICT"/></label>
		  <div class="col-sm-6">
			<form:select path="" id="ddSubdistrictAtVillLvl" class="combofield"  style="width:200px" onchange="getVillageListAtVillageLvl(this.value)" htmlEscape="true"></form:select>
		  </div>
		</div>
	</div>
		...
		<div class="ms_container row"  >
					...
					<div class="ms_selection col-sm-5">
						<div class="form-group">
							...
							<form:select path="villageIds" id="ddTargetDistrictVillLvl" multiple="true"  class="form-control" htmlEscape="true"/>
							<label id="err_village" class="mandatory"></label>
						</div>
					</div>
		</div>	
	</div>
	...
	<div class="ms_container row" >
		...
		<div class="ms_selection col-sm-5">
			<div class="form-group">
				...
				<label id="err_Administrativeblk" class="mandatory"></label>	
			</div>
		</div>
	</div>	
</div>
       ...
       <div class="pull-right">
       		<c:choose>
				<c:when test="${addStateDpetForm}">
					<button type="button" class="btn btn-success" id="btnSaveDept" onclick="javascript:processForm('Save');"><i class="fa fa-floppy-o"></i>Save</button>
					</c:when>
				<c:when test="${enableSaveBtn}">
					<button type="button" class="btn btn-success" id="btnSave" onclick="javascript:processForm('Save');"><i class="fa fa-floppy-o"></i>Save</button>
				</c:when>
				<c:otherwise>
					<button type="button" class="btn btn-success" id="btnCreateDeptNext"  onclick="javascript:processForm('Next');"><i class="fa fa-floppy-o"></i>Next</button>
				</c:otherwise>
			</c:choose>
       		<button type="button" class="btn btn-danger" id="close"  onclick="javascript:go('closeDepartmentProcess.htm');"><i class="fa fa-floppy-o"></i>Close</button>
        </div>
     </div>   
   </div>
</form:form>	
	...
	<div class="modal-dialog" role="document">
			<div class="modal-content">
  				<div class="modal-header">
    			  	...
    			  	<h4 class="modal-title" id="alertboxTitle">Confirm </h4>
  				</div>
  				<div class="modal-body" id="alertboxbody">
        
  				</div>
     			 <div class="modal-footer">
        		 <button type="button" id="alertboxbtnYES" class="btn btn-primary">YES</button>
        		 <button type="button" id="alertboxbtnNO" class="btn btn-primary">NO</button>
