...
<!--  COMMENTED TO RESOLVE LOST PAGES WHENEVER SOMEONE MOVED TO THE MENU  -->
<!-- 
...
<c:when test="${itm.parent eq 0}">
           ...
           <li class="myclass"><c:out value="${itm2.resourceId}" escapeXml="true"></c:out></li>
           </c:when>
           
           <c:when test="${itm2.formName  ne null and itm.groupId eq itm2.groupId and itm.appid eq itm2.appid}">
			<li ><a href="<%=getServletContext().getInitParameter("URL")%>/<c:out value="${itm2.formName}" escapeXml="true"/>?<csrf:token uri='${itm2.formName}'/>"><c:out value="${itm2.resourceId}" escapeXml="true"/></a></li>
			</c:when> 
