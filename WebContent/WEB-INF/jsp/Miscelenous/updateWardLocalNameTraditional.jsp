...
<%@include file="createPRIWardModificationJs.jsp"%>
...
$(document).ready(function() {
	...
	//$('#demand').DataTable();
	/* boolean ispresent = '${adminEntityDetail.ispresent}';
	if(ispresent)
		{
                    ...
                    <section class="col-lg-12">
                            <div class="box">
                                <div class="box-header with-border">
                                    <h3 class="box-title"><spring:message htmlEscape="true" code="Modify Ward Local Language Name Traditional" /></h3>
                                </div>
          <div class="box-body">
...
<%-- 							<th><spring:message    code="Label.Delete" htmlEscape="true" text="Delete Ward"></spring:message></th>
 --%>						</tr>
					...
					<c:forEach var="adminEntityDetail" varStatus="listAdminRow"	items="${wardList}">
							...
							<tr id="row_table"${listAdminRow.count}>
								...
								<td>  <input type="text" id="wardnumber${offsets*limits+(listAdminRow.index+1)}" name="wardnumber${offsets*limits+(listAdminRow.index+1)}"  size="10" maxlength="10" value="${adminEntityDetail.wardNumber}" readonly="true"
									  ...
									  <input type="hidden" id="wardnumberOld${offsets*limits+(listAdminRow.index+1)}" value="${adminEntityDetail.wardNumber}" />						
																	  ...
																	  <input type="hidden" id="wardCode${offsets*limits+(listAdminRow.index+1)}" value="${adminEntityDetail.wardCode}" />						
								
								</td>
								<td> <input type="text" id="wardname${offsets*limits+(listAdminRow.index+1)}" name ="wardName${offsets*limits+(listAdminRow.index+1)}" value="${adminEntityDetail.wardNameInEnglish}" readonly="true"
								...
								<td> <input type="text"	id="wardnamelocal${offsets*limits+(listAdminRow.index+1)}" name="wardnamelocal${offsets*limits+(listAdminRow.index+1)}" value="${adminEntityDetail.wardNameInLocal}"
														class="form-control" onfocus="validateOnFocus('wardnamelocal');changeErrorClass(this.id);"  
														maxlength="250"  
														onblur="validateSpecialCharactersWardNLocal(this.value);"  
														<c:if test="${adminEntityDetail.ischeck}">readonly </c:if>
														<c:if test="${adminEntityDetail.ischeck}">title="This Ward is in Draft Mode, Kindly Publish the Ward"</c:if>
													/>
									<input type="hidden" id="wardnamelocalOld${offsets*limits+(listAdminRow.index+1)}" value="${adminEntityDetail.wardNameInLocal}" />					
									<input type="hidden" id="wardVersion${offsets*limits+(listAdminRow.index+1)}" value="${adminEntityDetail.wardVersion}" />					
									<input type="hidden" id="ischeck${offsets*limits+(listAdminRow.index+1)}" value="${adminEntityDetail.ischeck}" />					
            ...
            <div class="box-footer">
                   <div class="col-sm-offset-2 col-sm-10">
                       <div class="pull-right">
                       		<input class="btn btn-success" type="button" id="btnSave" value="<spring:message code="Update" htmlEscape="true"></spring:message>" onclick="return saveEnabledvalues(); " />
