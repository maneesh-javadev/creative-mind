...
$(document).ready(function() {
	...
	$("#btnFormActionSaveDraft").click(function() {
		...
		callActionUrl('saveDraftGovermentOrder.htm');
	 });
 ...
 var validationForm = function (){
		$("#orderNo" ).rules( "add", {
	 	...
	 	<c:if test="${isGovernmentOrderUpload}">
	 		if($_checkEmptyObject($("#orderCode").val())){
	 			if( isParseJson("${not modifyProcess}") && isuploadNew){
	...
	<div class="form_stylings">
		...
		<div class="page_content">
			<div class="form_container">
				<form:form action="saveDraftGovermentOrder.htm" method="post" id="draftGovermentOrderForm" commandName="draftGovermentOrderForm" enctype="multipart/form-data">
				<input type="hidden" name="<csrf:token-name/>"	value="<csrf:token-value uri="saveDraftGovermentOrder.htm"/>" />
				<form:hidden path="paramCode"	id="paramCode"/>
	...
	<div class="col_1">
		...
		<ul class="form_body">
			...
			<li>
				...
				<fmt:formatDate var="orderDateValue" value="${draftGovermentOrderForm.orderDate}" pattern="dd/MM/yyyy" /> 
			...
			<li>
				...
				<fmt:formatDate var="effectiveDateValue" value="${draftGovermentOrderForm.effectiveDate}" pattern="dd/MM/yyyy" /> 
			...
			<c:choose>
				<c:when test="${isGovernmentOrderUpload}">
					<li>
						...
						<fmt:formatDate var="gazPubDateValue" value="${draftGovermentOrderForm.gazPubDate}" pattern="dd/MM/yyyy" /> 
					...
					<li>
						...
						<div id="divGazettePublicationUpload" <c:if test="${not empty draftGovermentOrderForm.orderCode}">style="display: none;"</c:if>>
							...
							<c:if test="${draftGovermentOrderForm.editMode}">
							...
							<tr>
							<td><c:out value="${draftGovermentOrderForm.fileName}" /></td>
							<td><c:out value="${draftGovermentOrderForm.fileSize}" /></td>
							<td><c:out value="${draftGovermentOrderForm.fileContentType}" /></td>
							<td>
