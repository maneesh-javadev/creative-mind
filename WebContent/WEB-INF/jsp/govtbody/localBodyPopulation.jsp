...
function getFreezeLocalbodybyState(){
	...
	if(validateForm()){
		lgdDwrStateDistrictFreezeService.getFreezeLocalbodybyStatePopulation(parseInt(parantLocal),parseInt(res[0]),parseInt(res[2]),parseInt(0),callBackData);
	}else{
		$("#tableId").hide();
		$("#save").hide();
			...
			$('#errorinter').html("");
			 $("#subdistrict").DataTable().destroy();
				...
				for(i=0;i<len;i++){
					...
					var td3=$("<td style='width: 70%'>"+data[i].localBodyCoverage+"</td>");
					var populationCount = 0;
					if(data[i].population != null)
						populationCount = data[i].population;
					var td4=$("<td><input type='number' min='0' style='text-align: center' name='lbFreezeUnfreezes["+i+"].population' placeholder='No. Of Population' value="+populationCount+"><input type='hidden' name='lbFreezeUnfreezes["+i+"].lblc' value="+data[i].lblc+"></td>");
...
function validateForm(){
	...
	var districtPanchyat = $('#distLevel').val();
	var intermediatePanchayat = $('#interLevel').val();
	if(localBodyType == 0 || districtPanchyat == 0 || intermediatePanchayat == 0 || intermediatePanchayat == null){
		return false;
	}return true;
}
...
function checkForNotSelectedValues(){
	...
	if(districtPanchyat == 0 ){
		$('#errordistrict').html("Please select district panchyat");
	}else{
		$('#errordistrict').html("");
	} 
	if(intermediatePanchayat == 0 || intermediatePanchayat == null){
		$('#errorinter').html("Please select intermediate panchyat");
	}else{
		$('#errorinter').html("");
	}
}
...
<c:set var="formTitle" value="Select Local Body Type"></c:set>
				...
				<div class="box">
					<form class="form-horizontal">
						...
						<div class="form-group">
					  		...
					  		<div class="col-sm-6">
						  		<select class="form-control" varStatus="index" id="entity" onchange="showHideBlock(this.value);">
									...
									<c:forEach var="getLocalGovtSetupList" items="${getLocalGovtSetupList}" varStatus="index">
										<option value="${getLocalGovtSetupList.nomenclatureEnglish}" selected="selected">
					  	...
					  	<div class="form-group">
						  	<label  class="col-sm-3 control-label" for="distLevel">
						  	...
						  	<div class="col-sm-6">
						  		<select class="form-control" varStatus="index" id="distLevel" onchange="getIntegermediatePanchayatListbylblc(this.value)">
									<option value="0"><spring:message code="Label.SEL" htmlEscape="true"></spring:message></option>
								</select>
						  	</div>
						  	<div id="errordistrict" style="color: red;"></div>
						</div>
						
						<div class="form-group">
						  	<label  class="col-sm-3 control-label" for="interLevel">
								<spring:message code="Label.SELECT" htmlEscape="true">
						  	...
						  	<div class="col-sm-6">
						  		<select name="interLevel" class="form-control" varStatus="index" id="interLevel" onchange="setVillagePanchayatCode(this.value)">
									<option value="0"><spring:message code="Label.SEL" htmlEscape="true"></spring:message></option>
								</select>
							</div>
							<div id="errorinter" style="color: red;"></div>
						</div>
						...
						<div class="box-footer">
						   <div class="col-sm-offset-2 col-sm-10">
							  <div class="pull-right">
								...
								<button style="width: 80px;" type="button" name="Close" class="btn btn-danger" onclick="javascript:location.href='home.htm?<csrf:token uri='home.htm'/>';">
									<i class="fa fa-times-circle"></i> 
						...
						<form:form  method="POST" action="localBodyPopulation.htm" commandName="freezeForm">
							<input type="hidden" name="<csrf:token-name/>" value="<csrf:token-value uri="localBodyPopulation.htm"/>"/>
							<div id="tableId">
								<table id="subdistrict" class="data_grid" cellspacing="0" style="width: 95%; margin-left: 3%;">
									<thead>
										<tr>
											...
											<th style="text-align: center;" id="populationHeader">Population</th>
										</tr>
									</thead>
									<tbody>
