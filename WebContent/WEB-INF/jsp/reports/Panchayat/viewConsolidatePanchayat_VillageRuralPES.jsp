...
function goBack()
{
	...
	document.getElementById('form1').action = "rptBacktoParentConsolidateReportPES.do?<csrf:token uri='rptBacktoParentConsolidateReport.do'/>";
	document.getElementById('form1').submit();
}
				...
				<table class="table table-striped table-bordered dataTable" width="100%" align="center" id="tbl_with_brdr">
				  <thead>
					<tr>
						...
						<th style="text-align: right"><c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'P')}"><spring:message code="Label.NOOFCENSUSVILLAGESMAPPED" htmlEscape="true"></spring:message></c:if>
							  ...
							  <c:set var="index" value="${panchaytSetUpRow.index +1}" />
								...
								<tr class="tblRowB">
									...
									<c:choose><c:when test="${panchaytSetUp.childCount == null}"><c:out value="0"></c:out></c:when>
											<c:otherwise><a href="#" onclick="javascript:manageState1('rptConsolidateforPanbyLevelRuralPES.do',${consolidateReportLBRPT.state_code},'${consolidateReportLBRPT.lbtype}','M',${panchaytSetUp.localBodyCode});"><c:out value="${panchaytSetUp.childCount}" escapeXml="true"></c:out></a>	</c:otherwise>
									</c:choose>
									</td>
