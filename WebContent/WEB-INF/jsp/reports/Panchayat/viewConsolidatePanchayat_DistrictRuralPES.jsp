				...
				<table class="table table-striped table-bordered dataTable" width="100%" align="center" id="tbl_with_brdr">
				  <thead>
					<tr>
						...
						<th style="text-align: right;"><c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'P')}"><spring:message code="Label.NO.INTERPANRPT" htmlEscape="true"></spring:message></c:if>
						...
						<form:input path="financialYear" type="hidden"  />
							...
							<c:set var="index" value="${panchaytSetUpRow.index +1}" />
								...
								<tr>
									...
									<td width="15%" align="right"><c:choose><c:when test="${panchaytSetUp.childCount == 0}"><c:out value="N.A."></c:out></c:when>
										<c:otherwise><%-- 	<a href="rptConsolidateforPanbyIstLevel.do?selstate=${consolidateReportLBRPT.state_code}&type=${consolidateReportLBRPT.lbtype}&level=I&parentLBCode=${panchaytSetUp.localBodyCode}&<csrf:token uri='rptConsolidateforPanbyIstLevel.do'/>">
											...
											<a href="#" onclick="javascript:return manageState1('rptConsolidateforPanbyLevelRuralPES.do',${consolidateReportLBRPT.state_code},'${consolidateReportLBRPT.lbtype}','I',${panchaytSetUp.localBodyCode});"><c:out value="${panchaytSetUp.childCount}" escapeXml="true"></c:out></a>	
										</c:otherwise>
										</c:choose>
									</td>
									<td width="15%" align="right"><c:choose><c:when test="${panchaytSetUp.grandChildCount == 0}"><c:out value="N.A."></c:out></c:when>
										<%-- <c:otherwise> --%><%-- <a href="rptConsolidateforPanbyIstLevel.do?selstate=${consolidateReportLBRPT.state_code}&type=${consolidateReportLBRPT.lbtype}&level=V&parentLBCode=${panchaytSetUp.localBodyCode}&<csrf:token uri='rptConsolidateforPanbyIstLevel.do'/>">--%>
										 <c:when test="${panchaytSetUp.childCount == 0}"> 	
										 	<a href="#" onclick="javascript:manageState1('rptConsolidateforPanbyLevelRuralPES.do',${consolidateReportLBRPT.state_code},'${consolidateReportLBRPT.lbtype}','V',${panchaytSetUp.localBodyCode});"><c:out value="${panchaytSetUp.grandChildCount}" escapeXml="true"></c:out></a>  
										 </c:when>	
