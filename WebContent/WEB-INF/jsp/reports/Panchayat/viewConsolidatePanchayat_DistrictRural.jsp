     	...
     	<div class="box">
            <div class="box-header with-border">
              ...
              <c:if test="${!empty consolidateLBList }"><a id="showprint" class="pull-right" href="#"><img src='<%=contextPath%>/images/printer_icon.png' alt="Print" border="0" onclick="PrintDiv();" /></a></c:if>
                ...
                <!-- form start -->
                <form:form class="form-horizontal" commandName="consolidateReportLBRPT" id='form1' name="form1">
				...
				<table class="table table-striped table-bordered dataTable" width="100%" align="center" id="tbl_with_brdr">
				  <thead>
					<tr>
						...
						<th><spring:message code="Label.LGDCODE" htmlEscape="true"></spring:message></th>
						<th><spring:message code="Label.STATESPECIFICCODE" htmlEscape="true"></spring:message></th>
						<th><c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'P')}"><spring:message code="Label.DISTRICTLBRPT" htmlEscape="true"></spring:message></c:if>
						...
						<th><c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'P')}"><spring:message code="Label.DISTRICTLBRPTLOCAL" htmlEscape="true"></spring:message></c:if>
						...
						<th><c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'P')}"><spring:message code="Label.NO.INTERPANRPT" htmlEscape="true"></spring:message></c:if>
						...
						<th><c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'P')}"><spring:message code="Label.NO.VILLAGEPANRPT" htmlEscape="true"></spring:message></c:if>
						...
						<form:input path="financialYear" type="hidden"  />
							...
							<c:set var="index" value="${panchaytSetUpRow.index +1}" />
								...
								<tr>
									...
									<td width="15%" align="center"><c:out value="${panchaytSetUp.localBodyCode}" escapeXml="true"></c:out></td>
									...
									<td width="15%" align="left"><c:out value="${panchaytSetUp.localbodyNameLocal}" escapeXml="true"></c:out>
									</td>
									<td width="15%" align="center"><c:choose><c:when test="${panchaytSetUp.childCount == 0}"><c:out value="N.A."></c:out></c:when>
										<c:otherwise><%-- 	<a href="rptConsolidateforPanbyIstLevel.do?selstate=${consolidateReportLBRPT.state_code}&type=${consolidateReportLBRPT.lbtype}&level=I&parentLBCode=${panchaytSetUp.localBodyCode}&<csrf:token uri='rptConsolidateforPanbyIstLevel.do'/>">
											<c:out value="${panchaytSetUp.childCount}"></c:out> </a> --%>		
									 		<%-- <a href="#" onclick="javascript:return manageState1('rptConsolidateforPanbyIstLevelRural.do',${consolidateReportLBRPT.state_code},'${consolidateReportLBRPT.lbtype}','I',${panchaytSetUp.localBodyCode});"><c:out value="${panchaytSetUp.childCount}"></c:out> </a> --%>
											<a href="#" onclick="javascript:return manageState1('rptConsolidateforPanbyLevelRural.do',${consolidateReportLBRPT.state_code},'${consolidateReportLBRPT.lbtype}','I',${panchaytSetUp.localBodyCode});"><c:out value="${panchaytSetUp.childCount}" escapeXml="true"></c:out></a>	
										</c:otherwise>
										</c:choose>
									</td>
									<td width="15%" align="center"><c:choose><c:when test="${panchaytSetUp.grandChildCount == 0}"><c:out value="N.A."></c:out></c:when>
										<%-- <c:otherwise> --%><%-- <a href="rptConsolidateforPanbyIstLevel.do?selstate=${consolidateReportLBRPT.state_code}&type=${consolidateReportLBRPT.lbtype}&level=V&parentLBCode=${panchaytSetUp.localBodyCode}&<csrf:token uri='rptConsolidateforPanbyIstLevel.do'/>">--%>
										 <c:when test="${panchaytSetUp.childCount == 0}"> 	
										 	<a href="#" onclick="javascript:manageState1('rptConsolidateforPanbyLevelRural.do',${consolidateReportLBRPT.state_code},'${consolidateReportLBRPT.lbtype}','V',${panchaytSetUp.localBodyCode});"><c:out value="${panchaytSetUp.grandChildCount}" escapeXml="true"></c:out></a>  
										 </c:when>	
									...
									<%-- <td align="center" width="10%">
										<c:if test="${panchaytSetUp.childCount gt 0}">
											<img src="images/showMap.jpg" onclick="javascript:displayMap('${panchaytSetUp.localBodyCode}');" width="18" height="19" border="0" />
					...
					<tr>
						...
						<td width="23%" align="center"><c:out value="${consolidateReportLBRPT.ip_count}" escapeXml="true"></c:out></td>
						<td width="23%" align="center"><c:out value="${consolidateReportLBRPT.vp_count}" escapeXml="true"></c:out></td>
