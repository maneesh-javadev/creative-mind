...
function manageState1(url,subdistrict_code,stateid,district_code)
{
	dwr.util.setValue('stateId', stateid, {	escapeHtml : false	});
	dwr.util.setValue('subdistrict_code', subdistrict_code, {	escapeHtml : false	});
	...
	<section class="col-lg-12 prebox">
     	<div class="box">
            <div class="box-header with-border">
              ...
              <c:if test="${!empty consolidateLBList }"><a id="showprint" class="pull-right" href="#"><img alt="Print" src='<%=contextPath%>/images/printer_icon.png' border="0" onclick="PrintDiv();" /></a></c:if>
				...
				<form:input path="stateId" type="hidden" name="stateId" id="stateId"  />
				 ...
				 <form:input path="parentlevel" type="hidden" name="parentlevel" id="parentlevel" value="D"/> 
						
			<div class="box-body" id="t1">
			...
			<h3 id="statusmessage" class="title" style="font-size:18px; font-weight:bold; text-align:center; margin:0;">
				...
				<spring:message code="label.consolidate.landregion.subdistrict" arguments="${consolidateReportLBRPT.entityNames}" argumentSeparator=","></spring:message>
					<%-- <<%-- /c:if>  --%><%-- <c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'T')}"> --%>
					<%-- <label><spring:message code="Label.DISTRICTLVLTRADLB" htmlEscape="true"></spring:message>&nbsp;<label><spring:message code="Label.OF" htmlEscape="true"></spring:message>&nbsp;${consolidateReportlb.state_name_english}</label>  --%>
					<%-- </c:if> --%>
		   	</h3>
				...
				<thead>
						<tr>
                            ...
                            <th width="10%" align="center"><label><spring:message code="Label.SUBDISTRICTCODE" htmlEscape="true"></spring:message> </label></th>
							<th width="27%" align="center">	<label><spring:message code="Label.SUBDISTRICTNAME" htmlEscape="true"></spring:message> </label></th>
							<th width="27%" align="center"><label><spring:message code="Label.SUBDISTRICTNAMELOCAL" htmlEscape="true"></spring:message> </label></th>
							...
							<th width="10%" align="center"><label> <spring:message code="Label.NOOFVILLAGES" htmlEscape="true"></spring:message> </label>
						...
						<c:if test="${! empty consolidateLBList}">
							<c:forEach var="panchaytSetUp" varStatus="panchaytSetUpRow" items="${consolidateLBList}">
								<tr>
									...
									<td align="center"><c:out value="${panchaytSetUp.subdistrict_code}" escapeXml="true"></c:out></td>
									<td align="left"><c:out value="${panchaytSetUp.subdistrict_name_english}" escapeXml="true"></c:out></td>
									<td align="left"><c:out value="${panchaytSetUp.subdistrict_name_local}" escapeXml="true"></c:out></td>
									...
									<td  align="center">
										<c:choose>
											<c:when test="${panchaytSetUp.noofvillages == 0}"><c:out value="N.A."></c:out></c:when>
											<c:otherwise>
                                        		...
                                        		<a href="#" onclick="javascript:manageState1('rptConsolidateforvillageLevel.do',${panchaytSetUp.subdistrict_code},${consolidateReportLBRPT.stateId},${consolidateReportLBRPT.district_code});"><c:out value="${panchaytSetUp.noofvillages}" escapeXml="true"></c:out></a>
											</c:otherwise>
										</c:choose>
									</td>
								</tr>
