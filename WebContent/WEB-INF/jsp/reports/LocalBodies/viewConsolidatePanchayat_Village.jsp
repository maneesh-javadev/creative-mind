...
function showmappedVillageDetails(stateid,lbtype,level,localbodycode)
{
	...
	document.getElementById('form1').action ="rptConsolidateLbMappedVillage.do?<csrf:token uri='rptConsolidateLbMappedVillage.do'/>"; 
     	...
     	<div class="box">
            <div class="box-header with-border">
              ...
              <c:if test="${!empty consolidateLBList }"><a id="showprint" href="#" class="pull-right"><img src='<%=contextPath%>/images/printer_icon.png' alt="Print" border="0" onclick="PrintDiv();" /></a></c:if>
				...
				<form:input path="captchaAnswer" type="hidden" value="0" id="captchaAnswer"/> 
				<form:input path="captchaAnswers" type="hidden" value="0" id="captchaAnswers"/> 
				...
				<div class="box-body" id="printble">
					<h3 id="statusmessage" class="title" style="font-size:18px; font-weight:bold; text-align:center; margin:0;">
						<c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'R')}">
							<spring:message code="Label.VILLAGELVLLBIES" htmlEscape="true"></spring:message> &nbsp;
						...
						<c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'T')}">
							<spring:message code="Label.VILLAGELVLTRADLB" htmlEscape="true"></spring:message> &nbsp;
							<spring:message code="Label.OF" htmlEscape="true"></spring:message>&nbsp;<c:out value="${consolidateReportLBRPT.state_name_english}" escapeXml="true"></c:out>
						</c:if>
				</h3>
				  ...
				  <thead>
					<tr>
						...
						<th><c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'R')}"><spring:message code="Label.LOCALGOVTBODYNAME" htmlEscape="true"></spring:message></c:if>
							 <c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'T')}"><spring:message code="Label.VILLAGELVLTRADLB" htmlEscape="true"></spring:message></c:if>
						</th>
						<th><c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'R')}"><spring:message code="Label.LOCALBODYNAMEINLOCAL" htmlEscape="true"></spring:message></c:if>
							<c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'T')}"><spring:message code="Label.VILLAGELVLLBLOCAL" htmlEscape="true"></spring:message></c:if>
						</th>
						<th><spring:message code="Label.LOCALBODYTYPE" htmlEscape="true"></spring:message></th>
						<th><c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'R')}"><spring:message code="Label.NOOFCENSUSVILLAGESMAPPED" htmlEscape="true"></spring:message></c:if>
							<c:if test="${fn:containsIgnoreCase(consolidateReportLBRPT.lbtype,'T')}"><spring:message code="Label.NOOFCENSUSVILLAGESMAPPED" htmlEscape="true"></spring:message></c:if>
						</th>
						...
						<c:if test="${! empty consolidateLBList}">
							<c:forEach var="panchaytSetUp" varStatus="panchaytSetUpRow" items="${consolidateLBList}">
								<tr>
									...
									<td width="20%" align="left"><c:out value="${panchaytSetUp.localbodyNameEnglish}" escapeXml="true"></c:out></td>
									<td width="22%" align="left"><c:out value="${panchaytSetUp.localbodyNameLocal}" escapeXml="true"></c:out></td>
									<td width="22%" align="left"><c:out value="${panchaytSetUp.localbodyTypeName}" escapeXml="true"></c:out></td>
									<td width="23%" align="center">
									<c:choose>
										<c:when test="${panchaytSetUp.childCount == null}"><c:out value="0"></c:out></c:when>
										<c:otherwise>
											<a href="#" onclick="showmappedVillageDetails(${consolidateReportLBRPT.state_code},'${consolidateReportLBRPT.lbtype}','M',${panchaytSetUp.localBodyCode});"><c:out value="${panchaytSetUp.childCount}" escapeXml="true"></c:out> </a>	
										</c:otherwise>
									</c:choose>
									</td>
