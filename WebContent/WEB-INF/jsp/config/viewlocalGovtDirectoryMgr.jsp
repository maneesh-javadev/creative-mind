...
<script type="text/javascript" language="javascript">
	...
	function submitForm() {
		document.getElementById('lgdmfrm').action = "lgdmFormAction.htm";
...
<section class="content">
              <div class="row">
                   <section class="col-lg-12">
                       <div class="box">
               <form:form action="lgdmForm.htm" method="POST" id="lgdmfrm" commandName="configGovtOrderForm" class="form-horizontal">
			      <input type="hidden" name="<csrf:token-name/>" value="<csrf:token-value uri="lgdmFormAction.htm"/>" />
                    ...
                    <table class="table table-bordered table-hover">
                        <thead>
							<tr >
								...
								<th width="2%"></th>
								<th width="30%" colspan="4"><spring:message htmlEscape="true" code="Label.OPTIONS"></spring:message></th>
							</tr>
						</thead>
					<tbody>
                       <c:forEach var="administratorUnit" varStatus="administratorUnitRow" items="${configGovtOrderForm.listAdminUnits}">
							<tr align="left" class="tblRowB">
								<spring:bind path="configGovtOrderForm.listAdminUnits[${administratorUnitRow.index}].operationBlockValue">
									...
									<td width="25%"><c:out value="${administratorUnit.operationBlockName}" escapeXml="true"></c:out></td>
								</spring:bind>
								<spring:bind path="configGovtOrderForm.listAdminUnits[${administratorUnitRow.index}].isgovtorderuploadBlock">
									<td width="2%"><input type="radio" name="<c:out value="${status.expression}"/>" value="true" <c:if test="${status.value == true}">checked</c:if> /></td>
									<td width="20%"><spring:message htmlEscape="true"  code="Label.UGO"></spring:message></td>
									<td width="2%"><input type="radio" name="<c:out value="${status.expression}"/>" value="false" disabled="disabled" /></td>
