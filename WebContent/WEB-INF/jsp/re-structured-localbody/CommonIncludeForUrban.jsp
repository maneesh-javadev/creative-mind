...
<script type="text/javascript">
	...
	var validationForm = function (){
		hasFormError = false;
	 	...
	 	if($_checkEmptyObject(resetedCoverage) ||  $.parseJSON(resetedCoverage)){
	 		var checkboxCoverageElement = $( "INPUT[id^='checkboxCoverage']" );
		 	if(!($(checkboxCoverageElement).is( ':checked' ))){
	 	...
	 	/* <c:if test="${isGovernmentOrderUpload}">
	 		if($_checkEmptyObject($("#orderCode").val())){
	 			if(($_checkEmptyObject(resetedCoverage) ||  $.parseJSON(resetedCoverage)) && $.parseJSON("${not modifyProcess}")){
