...
$(document).ready(function() {
	...
	$( "#actionFetchWardDetails" ).click(function() {
		if(checkNotUrbanProcess()){
			...
			//alert("lbTypeHierarchylement"+$( lbTypeHierarchylement ).val());
				if( $_checkEmptyObject($( lbTypeHierarchylement ).val()) ){
					$(lbTypeHierarchylement).addClass("textbox_errormsg");
		...
		if( $_checkEmptyObject(selectedlocalBodyType) ){
			$(localBodyTypeElement).addClass("textbox_errormsg");
...
function saveEnabledvalues(){
	...
	var blankStatus=false;
  	 for(var k=0;k<=actualsize;k++){
  		$('#wardname'+k).removeClass("textbox_errormsg");
  		$('#wardnumber'+k).removeClass("textbox_errormsg");
  		$('#wardnamelocal'+k).removeClass("textbox_errormsg");
    		 ...
    		 --totallength;
         	 ...
         	 for(var k=0;k<totallength;k++){
  		        var wardName 	=  	$('#wardname'+startid).val();
        	    	if(wardName==''){
        	    	$('#wardname'+startid).addClass("textbox_errormsg");
        	    	status=true;
        	    	
        	    	}
     	        var wardnumber  = 	$('#wardnumber'+startid).val();
     	        	if(wardnumber==''){
        	    	$('#wardnumber'+startid).addClass("textbox_errormsg");
        	    	status=true;
        	    	}
	 ...
	 for(var i=1;i<=size;i++){
		   if($('#check'+i).is(":checked")){
     	       ...
     	       if(wardName==''){
        	    	$('#wardname'+i).addClass("textbox_errormsg");
        	    	status=true;
        	    }else{
   	    		 var wardNameOld 	=  	$('#wardnameOld'+i).val();
	    		 if (wardName.replace(/ /g,'').toUpperCase() == wardNameOld.replace(/ /g,'').toUpperCase()) {
     	       ...
     	       if(wardnumber==''){
        	    	$('#wardnumber'+i).addClass("textbox_errormsg");
        	    	status=true;
        	    }
	    ...
	    $('#formNominations').val(array_tableData);
		  ...
		  if(chmandate){
			   showDialogBox("Kindly Change in Ward Name english ");
  	    	   erroFlag=true;
		  }else{
			  if(status){
  	 		  		  ...
  	 		  		  var wardName 	=  	$('#wardname'+k).val();
  	 			           ...
  	 			           for(var l=(k+1);l<tablelength1;l++){
  	 			        	         ...
  	 			        	         var wardNamecheck 	=  	$('#wardname'+l).val();
  	 			        	        	 ...
  	 			        	        	 $('#wardnumber'+l).addClass("textbox_errormsg");
  	 			        	        	 $('#wardnumber'+k).addClass("textbox_errormsg");
  	 			        	        	 duplicateStaus=true;
  	 			        	         }   
  	 			        	         if (wardName.replace(/ /g,'').toUpperCase() == wardNamecheck.replace(/ /g,'').toUpperCase()) {
  	 			        	        	 $('#wardname'+l).addClass("textbox_errormsg");
  	 			        	        	 $('#wardname'+k).addClass("textbox_errormsg");
  	 			        	        	 duplicateStaus=true;
  	 			        	         }
  	 		...
  	 		if(blankStatus==true){
  	 		 showDialogBox("kindly insert proper value");
  	 		}  
  	 		else if(duplicateStaus){
  	 			...
  	 			lgdDwrWardService.checkWardDuplicateInDraft(parseInt('${lblc}'),wardNames, {
  	 				callback : function(result) {
  	 					...
  	 					showDialogBox("Ward Name already exist in draft");
  	 						...
  	 						lgdDwrWardService.checkWardDuplicateInDeletedWard(parseInt('${lblc}'),wardNames, {
  	 		  	 				callback : function(result) {
  	 		  	 					//alert(result);
  	 		  	 					if(result){
  	 		  	 					showDialogBox("Ward Name already exist in delete ward");
  	 		  	 					}
  	 		  	 					else{
  	 		  	 						if(document.getElementById('UniqueWardNameError').innerHTML=="" && document.getElementById('UniqueWardCodeError').innerHTML==""){
  	 		  	 		  	 			
  	 		  	 							callActionUrl('createWard.htm','wardForm');
  	 		  	 		  	 		  	}
  	 		  	 					}
  	 		  	 				},
  	 		  	 				async : false
...
ClearForm=function(){
	callActionUrl('viewWardPRIAndTRI.htm','wardForm'); 
 };
</script>
