...
app.controller("changeEffectiveDateController",['$scope','$interval','$modal','changeEffectiveDateService',function($scope,$interval,$modal,changeEffectiveDateService){
	...
	$scope.changeEntityList=[];
	 $scope.entityList=[];
	 $scope.StateCode=_JS_STATE_CODE;
	 ...
	 loadinit();
		 ...
		 function loadinit(){
		 $scope.changeEntityList=[];
		 changeEffectiveDateService.getEntityDetail($scope.StateCode).then(function(response){
			 $scope.entityList= response.data.entityEffectiveList;
			 $scope.State_Detail= response.data.entityDetail;
	...
	$scope.showMinorVersion=function(villageName,villageCode,villageVersion){
		...
		reportModel= {"entityNameEnglish":villageName,"entityCode":villageCode, "entityVersion" : villageVersion,"entityType":'S'};
		
		var modalInstance = $modal.open({
			templateUrl: 'angularjs/changeEffectiveDate/state/minorListofEntity.html', 
