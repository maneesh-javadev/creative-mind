...
app.controller("changeEffectiveDateController",['$scope','$interval','$modal','changeEffectiveDateService',function($scope,$interval,$modal,changeEffectiveDateService){
	...
	$scope.changeEntityList=[];
	 $scope.entityList=[];
	 $scope.villageCode=_JS_VILLAGE_CODE;
	 ...
	 loadinit();
		 ...
		 function loadinit(){
		 $scope.changeEntityList=[];
		 changeEffectiveDateService.getEntityDetail($scope.villageCode).then(function(response){
			 $scope.entityList= response.data.entityEffectiveList;
			 $scope.villageDetail= response.data.entityDetail;
	...
	$scope.showMinorVersion=function(villageName,villageCode,villageVersion){
		...
		reportModel= {"entityNameEnglish":villageName,"entityCode":villageCode, "entityVersion" : villageVersion,"entityType":'V'};
		
		var modalInstance = $modal.open({
			templateUrl: 'angularjs/changeEffectiveDate/minorListofEntity.html', 
