...
app.controller("changeEffectiveDateController",['$scope','$interval','$modal','changeEffectiveDateService',function($scope,$interval,$modal,changeEffectiveDateService){
	...
	$scope.changeEntityList=[];
	 $scope.entityList=[];
	 $scope.DistrictCode=_JS_DISTRICT_CODE;
	 ...
	 loadinit();
		 ...
		 function loadinit(){
		 $scope.changeEntityList=[];
		 changeEffectiveDateService.getEntityDetail($scope.DistrictCode).then(function(response){
			 $scope.entityList= response.data.entityEffectiveList;
			 $scope.District_Detail= response.data.entityDetail;
	...
	$scope.showMinorVersion=function(villageName,villageCode,villageVersion){
		...
		reportModel= {"entityNameEnglish":villageName,"entityCode":villageCode, "entityVersion" : villageVersion,"entityType":'D'};
		
		var modalInstance = $modal.open({
			templateUrl: 'angularjs/changeEffectiveDate/district/minorListofEntity.html', 
