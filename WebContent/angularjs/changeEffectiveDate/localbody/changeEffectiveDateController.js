...
app.controller("changeEffectiveDateController",['$scope','$interval','$modal','changeEffectiveDateService',function($scope,$interval,$modal,changeEffectiveDateService){
	...
	$scope.changeEntityList=[];
	 ...
	 loadinit();
		 ...
		 changeEffectiveDateService.getEntityDetail($scope.lbCode).then(function(response){
			 ...
			 $scope.lbDetail= response.data.entityDetail;
	...
	$scope.showMinorVersion=function(villageName,villageCode,villageVersion){
		...
		reportModel= {"entityNameEnglish":villageName,"entityCode":villageCode, "entityVersion" : villageVersion,"entityType":'G'};
		
		var modalInstance = $modal.open({
			templateUrl: 'angularjs/changeEffectiveDate/localbody/minorListofEntity.html', 
